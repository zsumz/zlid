...
fn permute124(key: &[u8], tweak: &[u8], data124: u128) -> Result<u128> {
    if data124 > MASK_124 {
...
fn inverse_permute124(key: &[u8], tweak: &[u8], data124: u128) -> Result<u128> {
    if data124 > MASK_124 {
        return Err(Error::OutOfRange("124-bit value out of range"));
    }
    let mut left = ((data124 >> 62) as u64) & MASK_62;
...
fn round_function(key: &[u8], tweak: &[u8], round: u8, right: u64) -> Result<u64> {
    if right > MASK_62 {
