...
fn normalize_alias_key(key: &[u8]) -> Result<&[u8]> {
    if key.is_empty() {
        return Err(Error::EmptyAliasKey);
    }
    Ok(key)
}
...
fn alias_tag_to_source_tag(tag: u8) -> Result<u8> {
    match tag {
        ...
        _ => Err(Error::InvalidTag {
            operation: "unalias",
            expected: "a ZLID-A tag",
            actual: tag,
        }),
    }
}

fn permute124(hmac: &HmacSha256, tweak: &[u8], data124: u128) -> u128 {
    debug_assert!(data124 <= MASK_124);
...
fn inverse_permute124(hmac: &HmacSha256, tweak: &[u8], data124: u128) -> u128 {
    debug_assert!(data124 <= MASK_124);
    let mut left = ((data124 >> 62) as u64) & MASK_62;
    let mut right = (data124 as u64) & MASK_62;
...
fn round_function(hmac: &HmacSha256, tweak: &[u8], round: u8, right: u64) -> u64 {
    debug_assert!(right <= MASK_62);
